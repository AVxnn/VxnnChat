        ...
        <>
            ...
            <section ref={scrollRef} key={keyу} className={`message-section ${msg.from === auth.currentUser.uid ? 'me' : 'you'}`}>
                <section className='avatar-time'>
                    <Link to={`/profile/${msg.from !== auth.currentUser.uid ? msg.from : auth.currentUser.uid}`}><img className='message-avatar' src={`${msg.from === auth.currentUser.uid ? (auth.currentUser.photoURL ? auth.currentUser.photoURL : avatar) : (user2Avatar ? user2Avatar : avatar)}`} alt="avatar"/></Link>
